    ...
    this.setFileData(event, entity, field, isImage)
        ...
        public save(accion: 'borrador' | 'correcciones' | 'jurado'): void {
            ...
            try {
                ...
                this.proyecto.retroalimentacionAsesorEnviada = accion !== 'borrador';
                ...
                Promise.all(operaciones)
                    .then(() => {
                        if (accion === 'borrador') {
                            this.isSaving = false;
                            this.alertService().showAlert('Borrador guardado. El estudiante aún no puede ver la retroalimentación.', 'info');
                            return null;
                        }
                        if (accion === 'jurado') {
                            return this.enviarAlJurado();
                        }
                        this.isSaving = false;
                        this.alertService().showAlert('Correcciones enviadas al estudiante.', 'success');
                        return null;
                    })
                    .then(() => {
                        if (accion !== 'borrador') {
